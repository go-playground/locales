...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ro_RO'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ro *ro_RO) FmtDateTimeShort(t time.Time) []byte {

	b := ro.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ro.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ro_RO'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ro *ro_RO) FmtDateTimeMedium(t time.Time) []byte {

	b := ro.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ro.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ro_RO'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ro *ro_RO) FmtDateTimeLong(t time.Time) []byte {

	b := ro.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ro.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ro_RO'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ro *ro_RO) FmtDateTimeFull(t time.Time) []byte {

	b := ro.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ro.FmtTimeFull(t)...)
