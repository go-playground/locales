...
// FmtDateTimeShort returns the short date & time representation of 't' for 'lb_LU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lb *lb_LU) FmtDateTimeShort(t time.Time) []byte {

	b := lb.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, lb.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'lb_LU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lb *lb_LU) FmtDateTimeMedium(t time.Time) []byte {

	b := lb.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, lb.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'lb_LU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lb *lb_LU) FmtDateTimeLong(t time.Time) []byte {

	b := lb.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, lb.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'lb_LU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lb *lb_LU) FmtDateTimeFull(t time.Time) []byte {

	b := lb.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, lb.FmtTimeFull(t)...)
