	...
	for i := len(s) - 1; i >= 0; i-- {
		...
		if inWhole {
			...
			if count == 3 {
				...
				for j := len(ur.group) - 1; j >= 0; j-- {
					b = append(b, ur.group[j])
				}
