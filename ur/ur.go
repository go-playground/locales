...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ur'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ur *ur) FmtDateTimeShort(t time.Time) []byte {

	b := ur.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ur.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ur'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ur *ur) FmtDateTimeMedium(t time.Time) []byte {

	b := ur.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ur.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ur'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ur *ur) FmtDateTimeLong(t time.Time) []byte {

	b := ur.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ur.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ur'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ur *ur) FmtDateTimeFull(t time.Time) []byte {

	b := ur.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ur.FmtTimeFull(t)...)
