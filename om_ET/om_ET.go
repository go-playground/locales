...
// FmtDateTimeShort returns the short date & time representation of 't' for 'om_ET'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (om *om_ET) FmtDateTimeShort(t time.Time) []byte {

	b := om.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, om.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'om_ET'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (om *om_ET) FmtDateTimeMedium(t time.Time) []byte {

	b := om.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, om.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'om_ET'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (om *om_ET) FmtDateTimeLong(t time.Time) []byte {

	b := om.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, om.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'om_ET'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (om *om_ET) FmtDateTimeFull(t time.Time) []byte {

	b := om.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, om.FmtTimeFull(t)...)
