...
// FmtDateTimeShort returns the short date & time representation of 't' for 'br'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (br *br) FmtDateTimeShort(t time.Time) []byte {

	b := br.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, br.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'br'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (br *br) FmtDateTimeMedium(t time.Time) []byte {

	b := br.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, br.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'br'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (br *br) FmtDateTimeLong(t time.Time) []byte {

	b := br.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, br.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'br'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (br *br) FmtDateTimeFull(t time.Time) []byte {

	b := br.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, br.FmtTimeFull(t)...)
