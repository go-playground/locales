...
// FmtDateTimeShort returns the short date & time representation of 't' for 'en_AU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (en *en_AU) FmtDateTimeShort(t time.Time) []byte {

	b := en.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, en.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'en_AU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (en *en_AU) FmtDateTimeMedium(t time.Time) []byte {

	b := en.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, en.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'en_AU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (en *en_AU) FmtDateTimeLong(t time.Time) []byte {

	b := en.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, en.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'en_AU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (en *en_AU) FmtDateTimeFull(t time.Time) []byte {

	b := en.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, en.FmtTimeFull(t)...)
