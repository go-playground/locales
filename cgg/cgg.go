...
// FmtDateTimeShort returns the short date & time representation of 't' for 'cgg'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (cgg *cgg) FmtDateTimeShort(t time.Time) []byte {

	b := cgg.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, cgg.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'cgg'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (cgg *cgg) FmtDateTimeMedium(t time.Time) []byte {

	b := cgg.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, cgg.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'cgg'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (cgg *cgg) FmtDateTimeLong(t time.Time) []byte {

	b := cgg.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, cgg.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'cgg'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (cgg *cgg) FmtDateTimeFull(t time.Time) []byte {

	b := cgg.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, cgg.FmtTimeFull(t)...)
