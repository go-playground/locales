...
// FmtDateTimeShort returns the short date & time representation of 't' for 'gl'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (gl *gl) FmtDateTimeShort(t time.Time) []byte {

	b := gl.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, gl.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'gl'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (gl *gl) FmtDateTimeMedium(t time.Time) []byte {

	b := gl.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, gl.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'gl'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (gl *gl) FmtDateTimeLong(t time.Time) []byte {

	b := gl.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, gl.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'gl'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (gl *gl) FmtDateTimeFull(t time.Time) []byte {

	b := gl.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, gl.FmtTimeFull(t)...)
