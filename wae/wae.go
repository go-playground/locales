...
// FmtDateTimeShort returns the short date & time representation of 't' for 'wae'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (wae *wae) FmtDateTimeShort(t time.Time) []byte {

	b := wae.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, wae.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'wae'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (wae *wae) FmtDateTimeMedium(t time.Time) []byte {

	b := wae.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, wae.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'wae'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (wae *wae) FmtDateTimeLong(t time.Time) []byte {

	b := wae.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, wae.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'wae'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (wae *wae) FmtDateTimeFull(t time.Time) []byte {

	b := wae.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, wae.FmtTimeFull(t)...)
