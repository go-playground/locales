...
func (ta *ta_LK) FmtNumber(num float64, v uint64) []byte {
	...
	for i := len(s) - 1; i >= 0; i-- {
		...
		if s[i] == '.' {
			...
			for j := len(ta.decimal) - 1; j >= 0; j-- {
				b = append(b, ta.decimal[j])
			}
		...
		if inWhole {
			...
			if count == 3 {
				...
				for j := len(ta.group) - 1; j >= 0; j-- {
					b = append(b, ta.group[j])
				}
