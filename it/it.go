...
// FmtDateTimeShort returns the short date & time representation of 't' for 'it'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (it *it) FmtDateTimeShort(t time.Time) []byte {

	b := it.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, it.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'it'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (it *it) FmtDateTimeMedium(t time.Time) []byte {

	b := it.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, it.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'it'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (it *it) FmtDateTimeLong(t time.Time) []byte {

	b := it.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, it.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'it'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (it *it) FmtDateTimeFull(t time.Time) []byte {

	b := it.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, it.FmtTimeFull(t)...)
