...
// FmtDateTimeShort returns the short date & time representation of 't' for 'sg_CF'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sg *sg_CF) FmtDateTimeShort(t time.Time) []byte {

	b := sg.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, sg.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'sg_CF'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sg *sg_CF) FmtDateTimeMedium(t time.Time) []byte {

	b := sg.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, sg.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'sg_CF'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sg *sg_CF) FmtDateTimeLong(t time.Time) []byte {

	b := sg.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, sg.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'sg_CF'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sg *sg_CF) FmtDateTimeFull(t time.Time) []byte {

	b := sg.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, sg.FmtTimeFull(t)...)
