...
// FmtDateTimeShort returns the short date & time representation of 't' for 'id_ID'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (id *id_ID) FmtDateTimeShort(t time.Time) []byte {

	b := id.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, id.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'id_ID'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (id *id_ID) FmtDateTimeMedium(t time.Time) []byte {

	b := id.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, id.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'id_ID'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (id *id_ID) FmtDateTimeLong(t time.Time) []byte {

	b := id.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, id.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'id_ID'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (id *id_ID) FmtDateTimeFull(t time.Time) []byte {

	b := id.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, id.FmtTimeFull(t)...)
