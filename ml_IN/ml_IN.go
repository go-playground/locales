...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ml_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ml *ml_IN) FmtDateTimeShort(t time.Time) []byte {

	b := ml.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ml.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ml_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ml *ml_IN) FmtDateTimeMedium(t time.Time) []byte {

	b := ml.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ml.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ml_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ml *ml_IN) FmtDateTimeLong(t time.Time) []byte {

	b := ml.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ml.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ml_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ml *ml_IN) FmtDateTimeFull(t time.Time) []byte {

	b := ml.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ml.FmtTimeFull(t)...)
