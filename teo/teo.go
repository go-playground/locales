...
// FmtDateTimeShort returns the short date & time representation of 't' for 'teo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (teo *teo) FmtDateTimeShort(t time.Time) []byte {

	b := teo.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, teo.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'teo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (teo *teo) FmtDateTimeMedium(t time.Time) []byte {

	b := teo.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, teo.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'teo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (teo *teo) FmtDateTimeLong(t time.Time) []byte {

	b := teo.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, teo.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'teo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (teo *teo) FmtDateTimeFull(t time.Time) []byte {

	b := teo.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, teo.FmtTimeFull(t)...)
