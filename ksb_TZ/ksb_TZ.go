...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ksb_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ksb *ksb_TZ) FmtDateTimeShort(t time.Time) []byte {

	b := ksb.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ksb.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ksb_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ksb *ksb_TZ) FmtDateTimeMedium(t time.Time) []byte {

	b := ksb.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ksb.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ksb_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ksb *ksb_TZ) FmtDateTimeLong(t time.Time) []byte {

	b := ksb.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ksb.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ksb_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ksb *ksb_TZ) FmtDateTimeFull(t time.Time) []byte {

	b := ksb.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ksb.FmtTimeFull(t)...)
