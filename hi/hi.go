...
// FmtDateTimeShort returns the short date & time representation of 't' for 'hi'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (hi *hi) FmtDateTimeShort(t time.Time) []byte {

	b := hi.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, hi.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'hi'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (hi *hi) FmtDateTimeMedium(t time.Time) []byte {

	b := hi.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, hi.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'hi'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (hi *hi) FmtDateTimeLong(t time.Time) []byte {

	b := hi.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, hi.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'hi'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (hi *hi) FmtDateTimeFull(t time.Time) []byte {

	b := hi.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, hi.FmtTimeFull(t)...)
