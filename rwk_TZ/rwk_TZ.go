...
// FmtDateTimeShort returns the short date & time representation of 't' for 'rwk_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rwk *rwk_TZ) FmtDateTimeShort(t time.Time) []byte {

	b := rwk.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, rwk.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'rwk_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rwk *rwk_TZ) FmtDateTimeMedium(t time.Time) []byte {

	b := rwk.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, rwk.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'rwk_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rwk *rwk_TZ) FmtDateTimeLong(t time.Time) []byte {

	b := rwk.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, rwk.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'rwk_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rwk *rwk_TZ) FmtDateTimeFull(t time.Time) []byte {

	b := rwk.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, rwk.FmtTimeFull(t)...)
