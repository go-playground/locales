...
// FmtDateTimeShort returns the short date & time representation of 't' for 'nyn'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nyn *nyn) FmtDateTimeShort(t time.Time) []byte {

	b := nyn.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, nyn.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'nyn'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nyn *nyn) FmtDateTimeMedium(t time.Time) []byte {

	b := nyn.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, nyn.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'nyn'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nyn *nyn) FmtDateTimeLong(t time.Time) []byte {

	b := nyn.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, nyn.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'nyn'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nyn *nyn) FmtDateTimeFull(t time.Time) []byte {

	b := nyn.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, nyn.FmtTimeFull(t)...)
