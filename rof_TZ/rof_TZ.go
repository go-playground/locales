...
// FmtDateTimeShort returns the short date & time representation of 't' for 'rof_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rof *rof_TZ) FmtDateTimeShort(t time.Time) []byte {

	b := rof.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, rof.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'rof_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rof *rof_TZ) FmtDateTimeMedium(t time.Time) []byte {

	b := rof.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, rof.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'rof_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rof *rof_TZ) FmtDateTimeLong(t time.Time) []byte {

	b := rof.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, rof.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'rof_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rof *rof_TZ) FmtDateTimeFull(t time.Time) []byte {

	b := rof.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, rof.FmtTimeFull(t)...)
