...
// FmtDateTimeShort returns the short date & time representation of 't' for 'guz_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (guz *guz_KE) FmtDateTimeShort(t time.Time) []byte {

	b := guz.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, guz.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'guz_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (guz *guz_KE) FmtDateTimeMedium(t time.Time) []byte {

	b := guz.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, guz.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'guz_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (guz *guz_KE) FmtDateTimeLong(t time.Time) []byte {

	b := guz.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, guz.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'guz_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (guz *guz_KE) FmtDateTimeFull(t time.Time) []byte {

	b := guz.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, guz.FmtTimeFull(t)...)
