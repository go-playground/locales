...
func (ja *ja) FmtNumber(num float64, v uint64) []byte {
	...
	for i := len(s) - 1; i >= 0; i-- {
		...
		if s[i] == '.' {
			...
			for j := len(ja.decimal) - 1; j >= 0; j-- {
				b = append(b, ja.decimal[j])
			}
		...
		if inWhole {
			...
			if count == 3 {
				...
				for j := len(ja.group) - 1; j >= 0; j-- {
					b = append(b, ja.group[j])
				}
