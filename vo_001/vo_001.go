...
// FmtDateTimeShort returns the short date & time representation of 't' for 'vo_001'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (vo *vo_001) FmtDateTimeShort(t time.Time) []byte {

	b := vo.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, vo.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'vo_001'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (vo *vo_001) FmtDateTimeMedium(t time.Time) []byte {

	b := vo.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, vo.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'vo_001'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (vo *vo_001) FmtDateTimeLong(t time.Time) []byte {

	b := vo.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, vo.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'vo_001'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (vo *vo_001) FmtDateTimeFull(t time.Time) []byte {

	b := vo.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, vo.FmtTimeFull(t)...)
