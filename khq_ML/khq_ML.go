...
// FmtDateTimeShort returns the short date & time representation of 't' for 'khq_ML'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (khq *khq_ML) FmtDateTimeShort(t time.Time) []byte {

	b := khq.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, khq.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'khq_ML'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (khq *khq_ML) FmtDateTimeMedium(t time.Time) []byte {

	b := khq.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, khq.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'khq_ML'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (khq *khq_ML) FmtDateTimeLong(t time.Time) []byte {

	b := khq.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, khq.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'khq_ML'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (khq *khq_ML) FmtDateTimeFull(t time.Time) []byte {

	b := khq.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, khq.FmtTimeFull(t)...)
