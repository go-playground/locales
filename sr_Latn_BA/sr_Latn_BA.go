...
// FmtDateTimeShort returns the short date & time representation of 't' for 'sr_Latn_BA'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sr *sr_Latn_BA) FmtDateTimeShort(t time.Time) []byte {

	b := sr.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, sr.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'sr_Latn_BA'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sr *sr_Latn_BA) FmtDateTimeMedium(t time.Time) []byte {

	b := sr.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, sr.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'sr_Latn_BA'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sr *sr_Latn_BA) FmtDateTimeLong(t time.Time) []byte {

	b := sr.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, sr.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'sr_Latn_BA'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sr *sr_Latn_BA) FmtDateTimeFull(t time.Time) []byte {

	b := sr.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, sr.FmtTimeFull(t)...)
