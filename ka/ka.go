...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ka'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ka *ka) FmtDateTimeShort(t time.Time) []byte {

	b := ka.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ka.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ka'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ka *ka) FmtDateTimeMedium(t time.Time) []byte {

	b := ka.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ka.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ka'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ka *ka) FmtDateTimeLong(t time.Time) []byte {

	b := ka.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ka.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ka'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ka *ka) FmtDateTimeFull(t time.Time) []byte {

	b := ka.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ka.FmtTimeFull(t)...)
