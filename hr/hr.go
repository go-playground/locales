...
// FmtDateTimeShort returns the short date & time representation of 't' for 'hr'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (hr *hr) FmtDateTimeShort(t time.Time) []byte {

	b := hr.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, hr.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'hr'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (hr *hr) FmtDateTimeMedium(t time.Time) []byte {

	b := hr.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, hr.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'hr'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (hr *hr) FmtDateTimeLong(t time.Time) []byte {

	b := hr.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, hr.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'hr'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (hr *hr) FmtDateTimeFull(t time.Time) []byte {

	b := hr.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, hr.FmtTimeFull(t)...)
