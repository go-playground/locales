...
// FmtDateTimeShort returns the short date & time representation of 't' for 'nmg'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nmg *nmg) FmtDateTimeShort(t time.Time) []byte {

	b := nmg.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, nmg.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'nmg'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nmg *nmg) FmtDateTimeMedium(t time.Time) []byte {

	b := nmg.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, nmg.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'nmg'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nmg *nmg) FmtDateTimeLong(t time.Time) []byte {

	b := nmg.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, nmg.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'nmg'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nmg *nmg) FmtDateTimeFull(t time.Time) []byte {

	b := nmg.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, nmg.FmtTimeFull(t)...)
