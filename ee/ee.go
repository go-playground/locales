...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ee'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ee *ee) FmtDateTimeShort(t time.Time) []byte {

	b := ee.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ee.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ee'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ee *ee) FmtDateTimeMedium(t time.Time) []byte {

	b := ee.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ee.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ee'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ee *ee) FmtDateTimeLong(t time.Time) []byte {

	b := ee.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ee.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ee'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ee *ee) FmtDateTimeFull(t time.Time) []byte {

	b := ee.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ee.FmtTimeFull(t)...)
