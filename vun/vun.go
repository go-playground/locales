...
// FmtDateTimeShort returns the short date & time representation of 't' for 'vun'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (vun *vun) FmtDateTimeShort(t time.Time) []byte {

	b := vun.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, vun.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'vun'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (vun *vun) FmtDateTimeMedium(t time.Time) []byte {

	b := vun.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, vun.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'vun'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (vun *vun) FmtDateTimeLong(t time.Time) []byte {

	b := vun.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, vun.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'vun'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (vun *vun) FmtDateTimeFull(t time.Time) []byte {

	b := vun.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, vun.FmtTimeFull(t)...)
