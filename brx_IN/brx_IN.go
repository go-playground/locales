...
// FmtDateTimeShort returns the short date & time representation of 't' for 'brx_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (brx *brx_IN) FmtDateTimeShort(t time.Time) []byte {

	b := brx.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, brx.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'brx_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (brx *brx_IN) FmtDateTimeMedium(t time.Time) []byte {

	b := brx.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, brx.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'brx_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (brx *brx_IN) FmtDateTimeLong(t time.Time) []byte {

	b := brx.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, brx.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'brx_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (brx *brx_IN) FmtDateTimeFull(t time.Time) []byte {

	b := brx.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, brx.FmtTimeFull(t)...)
