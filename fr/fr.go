...
// FmtDateTimeShort returns the short date & time representation of 't' for 'fr'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fr *fr) FmtDateTimeShort(t time.Time) []byte {

	b := fr.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, fr.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'fr'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fr *fr) FmtDateTimeMedium(t time.Time) []byte {

	b := fr.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, fr.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'fr'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fr *fr) FmtDateTimeLong(t time.Time) []byte {

	b := fr.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, fr.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'fr'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fr *fr) FmtDateTimeFull(t time.Time) []byte {

	b := fr.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, fr.FmtTimeFull(t)...)
