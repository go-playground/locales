...
// FmtDateTimeShort returns the short date & time representation of 't' for 'cy_GB'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (cy *cy_GB) FmtDateTimeShort(t time.Time) []byte {

	b := cy.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, cy.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'cy_GB'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (cy *cy_GB) FmtDateTimeMedium(t time.Time) []byte {

	b := cy.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, cy.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'cy_GB'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (cy *cy_GB) FmtDateTimeLong(t time.Time) []byte {

	b := cy.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, cy.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'cy_GB'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (cy *cy_GB) FmtDateTimeFull(t time.Time) []byte {

	b := cy.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, cy.FmtTimeFull(t)...)
