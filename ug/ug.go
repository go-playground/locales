...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ug'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ug *ug) FmtDateTimeShort(t time.Time) []byte {

	b := ug.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ug.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ug'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ug *ug) FmtDateTimeMedium(t time.Time) []byte {

	b := ug.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ug.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ug'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ug *ug) FmtDateTimeLong(t time.Time) []byte {

	b := ug.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ug.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ug'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ug *ug) FmtDateTimeFull(t time.Time) []byte {

	b := ug.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ug.FmtTimeFull(t)...)
