...
// FmtDateTimeShort returns the short date & time representation of 't' for 'nus'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nus *nus) FmtDateTimeShort(t time.Time) []byte {

	b := nus.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, nus.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'nus'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nus *nus) FmtDateTimeMedium(t time.Time) []byte {

	b := nus.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, nus.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'nus'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nus *nus) FmtDateTimeLong(t time.Time) []byte {

	b := nus.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, nus.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'nus'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nus *nus) FmtDateTimeFull(t time.Time) []byte {

	b := nus.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, nus.FmtTimeFull(t)...)
