...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ru_MD'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ru *ru_MD) FmtDateTimeShort(t time.Time) []byte {

	b := ru.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ru.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ru_MD'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ru *ru_MD) FmtDateTimeMedium(t time.Time) []byte {

	b := ru.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ru.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ru_MD'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ru *ru_MD) FmtDateTimeLong(t time.Time) []byte {

	b := ru.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ru.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ru_MD'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ru *ru_MD) FmtDateTimeFull(t time.Time) []byte {

	b := ru.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ru.FmtTimeFull(t)...)
