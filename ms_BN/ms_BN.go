...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ms_BN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ms *ms_BN) FmtDateTimeShort(t time.Time) []byte {

	b := ms.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ms.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ms_BN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ms *ms_BN) FmtDateTimeMedium(t time.Time) []byte {

	b := ms.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ms.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ms_BN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ms *ms_BN) FmtDateTimeLong(t time.Time) []byte {

	b := ms.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ms.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ms_BN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ms *ms_BN) FmtDateTimeFull(t time.Time) []byte {

	b := ms.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ms.FmtTimeFull(t)...)
