...
// FmtDateTimeShort returns the short date & time representation of 't' for 'pl'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (pl *pl) FmtDateTimeShort(t time.Time) []byte {

	b := pl.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, pl.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'pl'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (pl *pl) FmtDateTimeMedium(t time.Time) []byte {

	b := pl.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, pl.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'pl'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (pl *pl) FmtDateTimeLong(t time.Time) []byte {

	b := pl.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, pl.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'pl'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (pl *pl) FmtDateTimeFull(t time.Time) []byte {

	b := pl.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, pl.FmtTimeFull(t)...)
