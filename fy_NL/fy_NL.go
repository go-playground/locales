...
// FmtDateTimeShort returns the short date & time representation of 't' for 'fy_NL'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fy *fy_NL) FmtDateTimeShort(t time.Time) []byte {

	b := fy.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, fy.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'fy_NL'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fy *fy_NL) FmtDateTimeMedium(t time.Time) []byte {

	b := fy.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, fy.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'fy_NL'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fy *fy_NL) FmtDateTimeLong(t time.Time) []byte {

	b := fy.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, fy.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'fy_NL'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fy *fy_NL) FmtDateTimeFull(t time.Time) []byte {

	b := fy.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, fy.FmtTimeFull(t)...)
