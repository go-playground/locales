...
// FmtDateTimeShort returns the short date & time representation of 't' for 'is'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (is *is) FmtDateTimeShort(t time.Time) []byte {

	b := is.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, is.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'is'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (is *is) FmtDateTimeMedium(t time.Time) []byte {

	b := is.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, is.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'is'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (is *is) FmtDateTimeLong(t time.Time) []byte {

	b := is.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, is.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'is'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (is *is) FmtDateTimeFull(t time.Time) []byte {

	b := is.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, is.FmtTimeFull(t)...)
