...
// FmtDateTimeShort returns the short date & time representation of 't' for 'rw'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rw *rw) FmtDateTimeShort(t time.Time) []byte {

	b := rw.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, rw.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'rw'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rw *rw) FmtDateTimeMedium(t time.Time) []byte {

	b := rw.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, rw.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'rw'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rw *rw) FmtDateTimeLong(t time.Time) []byte {

	b := rw.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, rw.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'rw'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rw *rw) FmtDateTimeFull(t time.Time) []byte {

	b := rw.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, rw.FmtTimeFull(t)...)
