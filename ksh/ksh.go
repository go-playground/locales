...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ksh'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ksh *ksh) FmtDateTimeShort(t time.Time) []byte {

	b := ksh.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ksh.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ksh'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ksh *ksh) FmtDateTimeMedium(t time.Time) []byte {

	b := ksh.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ksh.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ksh'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ksh *ksh) FmtDateTimeLong(t time.Time) []byte {

	b := ksh.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ksh.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ksh'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ksh *ksh) FmtDateTimeFull(t time.Time) []byte {

	b := ksh.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ksh.FmtTimeFull(t)...)
