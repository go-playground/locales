...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ce_RU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ce *ce_RU) FmtDateTimeShort(t time.Time) []byte {

	b := ce.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ce.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ce_RU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ce *ce_RU) FmtDateTimeMedium(t time.Time) []byte {

	b := ce.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ce.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ce_RU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ce *ce_RU) FmtDateTimeLong(t time.Time) []byte {

	b := ce.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ce.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ce_RU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ce *ce_RU) FmtDateTimeFull(t time.Time) []byte {

	b := ce.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ce.FmtTimeFull(t)...)
