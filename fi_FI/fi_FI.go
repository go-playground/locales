...
// FmtDateTimeShort returns the short date & time representation of 't' for 'fi_FI'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fi *fi_FI) FmtDateTimeShort(t time.Time) []byte {

	b := fi.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, fi.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'fi_FI'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fi *fi_FI) FmtDateTimeMedium(t time.Time) []byte {

	b := fi.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, fi.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'fi_FI'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fi *fi_FI) FmtDateTimeLong(t time.Time) []byte {

	b := fi.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, fi.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'fi_FI'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fi *fi_FI) FmtDateTimeFull(t time.Time) []byte {

	b := fi.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, fi.FmtTimeFull(t)...)
