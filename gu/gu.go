...
// FmtDateTimeShort returns the short date & time representation of 't' for 'gu'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (gu *gu) FmtDateTimeShort(t time.Time) []byte {

	b := gu.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, gu.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'gu'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (gu *gu) FmtDateTimeMedium(t time.Time) []byte {

	b := gu.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, gu.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'gu'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (gu *gu) FmtDateTimeLong(t time.Time) []byte {

	b := gu.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, gu.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'gu'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (gu *gu) FmtDateTimeFull(t time.Time) []byte {

	b := gu.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, gu.FmtTimeFull(t)...)
