...
// FmtDateTimeShort returns the short date & time representation of 't' for 'root'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (root *root) FmtDateTimeShort(t time.Time) []byte {

	b := root.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, root.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'root'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (root *root) FmtDateTimeMedium(t time.Time) []byte {

	b := root.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, root.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'root'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (root *root) FmtDateTimeLong(t time.Time) []byte {

	b := root.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, root.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'root'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (root *root) FmtDateTimeFull(t time.Time) []byte {

	b := root.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, root.FmtTimeFull(t)...)
