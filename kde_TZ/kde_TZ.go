...
// FmtDateTimeShort returns the short date & time representation of 't' for 'kde_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kde *kde_TZ) FmtDateTimeShort(t time.Time) []byte {

	b := kde.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, kde.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'kde_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kde *kde_TZ) FmtDateTimeMedium(t time.Time) []byte {

	b := kde.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, kde.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'kde_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kde *kde_TZ) FmtDateTimeLong(t time.Time) []byte {

	b := kde.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, kde.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'kde_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kde *kde_TZ) FmtDateTimeFull(t time.Time) []byte {

	b := kde.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, kde.FmtTimeFull(t)...)
