...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ks_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ks *ks_IN) FmtDateTimeShort(t time.Time) []byte {

	b := ks.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ks.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ks_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ks *ks_IN) FmtDateTimeMedium(t time.Time) []byte {

	b := ks.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ks.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ks_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ks *ks_IN) FmtDateTimeLong(t time.Time) []byte {

	b := ks.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ks.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ks_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ks *ks_IN) FmtDateTimeFull(t time.Time) []byte {

	b := ks.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ks.FmtTimeFull(t)...)
