...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ksf_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ksf *ksf_CM) FmtDateTimeShort(t time.Time) []byte {

	b := ksf.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ksf.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ksf_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ksf *ksf_CM) FmtDateTimeMedium(t time.Time) []byte {

	b := ksf.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ksf.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ksf_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ksf *ksf_CM) FmtDateTimeLong(t time.Time) []byte {

	b := ksf.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ksf.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ksf_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ksf *ksf_CM) FmtDateTimeFull(t time.Time) []byte {

	b := ksf.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ksf.FmtTimeFull(t)...)
