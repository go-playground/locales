...
// FmtDateTimeShort returns the short date & time representation of 't' for 'tk'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (tk *tk) FmtDateTimeShort(t time.Time) []byte {

	b := tk.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, tk.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'tk'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (tk *tk) FmtDateTimeMedium(t time.Time) []byte {

	b := tk.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, tk.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'tk'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (tk *tk) FmtDateTimeLong(t time.Time) []byte {

	b := tk.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, tk.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'tk'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (tk *tk) FmtDateTimeFull(t time.Time) []byte {

	b := tk.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, tk.FmtTimeFull(t)...)
