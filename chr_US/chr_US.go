...
// FmtDateTimeShort returns the short date & time representation of 't' for 'chr_US'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (chr *chr_US) FmtDateTimeShort(t time.Time) []byte {

	b := chr.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, chr.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'chr_US'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (chr *chr_US) FmtDateTimeMedium(t time.Time) []byte {

	b := chr.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, chr.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'chr_US'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (chr *chr_US) FmtDateTimeLong(t time.Time) []byte {

	b := chr.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, chr.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'chr_US'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (chr *chr_US) FmtDateTimeFull(t time.Time) []byte {

	b := chr.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, chr.FmtTimeFull(t)...)
