...
// FmtDateTimeShort returns the short date & time representation of 't' for 'kw_GB'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kw *kw_GB) FmtDateTimeShort(t time.Time) []byte {

	b := kw.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, kw.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'kw_GB'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kw *kw_GB) FmtDateTimeMedium(t time.Time) []byte {

	b := kw.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, kw.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'kw_GB'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kw *kw_GB) FmtDateTimeLong(t time.Time) []byte {

	b := kw.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, kw.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'kw_GB'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kw *kw_GB) FmtDateTimeFull(t time.Time) []byte {

	b := kw.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, kw.FmtTimeFull(t)...)
