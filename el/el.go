...
// FmtDateTimeShort returns the short date & time representation of 't' for 'el'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (el *el) FmtDateTimeShort(t time.Time) []byte {

	b := el.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, el.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'el'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (el *el) FmtDateTimeMedium(t time.Time) []byte {

	b := el.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, el.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'el'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (el *el) FmtDateTimeLong(t time.Time) []byte {

	b := el.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, el.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'el'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (el *el) FmtDateTimeFull(t time.Time) []byte {

	b := el.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, el.FmtTimeFull(t)...)
