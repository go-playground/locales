...
// FmtDateTimeShort returns the short date & time representation of 't' for 'gsw'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (gsw *gsw) FmtDateTimeShort(t time.Time) []byte {

	b := gsw.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, gsw.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'gsw'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (gsw *gsw) FmtDateTimeMedium(t time.Time) []byte {

	b := gsw.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, gsw.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'gsw'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (gsw *gsw) FmtDateTimeLong(t time.Time) []byte {

	b := gsw.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, gsw.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'gsw'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (gsw *gsw) FmtDateTimeFull(t time.Time) []byte {

	b := gsw.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, gsw.FmtTimeFull(t)...)
