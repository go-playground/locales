...
// FmtDateTimeShort returns the short date & time representation of 't' for 'nb'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nb *nb) FmtDateTimeShort(t time.Time) []byte {

	b := nb.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, nb.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'nb'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nb *nb) FmtDateTimeMedium(t time.Time) []byte {

	b := nb.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, nb.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'nb'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nb *nb) FmtDateTimeLong(t time.Time) []byte {

	b := nb.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, nb.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'nb'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nb *nb) FmtDateTimeFull(t time.Time) []byte {

	b := nb.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, nb.FmtTimeFull(t)...)
