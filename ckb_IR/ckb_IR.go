...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ckb_IR'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ckb *ckb_IR) FmtDateTimeShort(t time.Time) []byte {

	b := ckb.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ckb.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ckb_IR'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ckb *ckb_IR) FmtDateTimeMedium(t time.Time) []byte {

	b := ckb.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ckb.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ckb_IR'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ckb *ckb_IR) FmtDateTimeLong(t time.Time) []byte {

	b := ckb.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ckb.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ckb_IR'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ckb *ckb_IR) FmtDateTimeFull(t time.Time) []byte {

	b := ckb.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ckb.FmtTimeFull(t)...)
