...
// FmtDateTimeShort returns the short date & time representation of 't' for 'lo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lo *lo) FmtDateTimeShort(t time.Time) []byte {

	b := lo.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, lo.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'lo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lo *lo) FmtDateTimeMedium(t time.Time) []byte {

	b := lo.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, lo.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'lo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lo *lo) FmtDateTimeLong(t time.Time) []byte {

	b := lo.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, lo.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'lo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lo *lo) FmtDateTimeFull(t time.Time) []byte {

	b := lo.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, lo.FmtTimeFull(t)...)
