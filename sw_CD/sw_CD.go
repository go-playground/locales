...
func (sw *sw_CD) FmtNumber(num float64, v uint64) []byte {
	...
	for i := len(s) - 1; i >= 0; i-- {
		...
		if s[i] == '.' {
			...
			for j := len(sw.decimal) - 1; j >= 0; j-- {
				b = append(b, sw.decimal[j])
			}
		...
		if inWhole {
			...
			if count == 3 {
				...
				for j := len(sw.group) - 1; j >= 0; j-- {
					b = append(b, sw.group[j])
				}
