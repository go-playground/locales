...
// FmtDateTimeShort returns the short date & time representation of 't' for 'tzm_MA'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (tzm *tzm_MA) FmtDateTimeShort(t time.Time) []byte {

	b := tzm.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, tzm.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'tzm_MA'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (tzm *tzm_MA) FmtDateTimeMedium(t time.Time) []byte {

	b := tzm.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, tzm.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'tzm_MA'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (tzm *tzm_MA) FmtDateTimeLong(t time.Time) []byte {

	b := tzm.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, tzm.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'tzm_MA'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (tzm *tzm_MA) FmtDateTimeFull(t time.Time) []byte {

	b := tzm.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, tzm.FmtTimeFull(t)...)
