...
// FmtDateTimeShort returns the short date & time representation of 't' for 'mua_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mua *mua_CM) FmtDateTimeShort(t time.Time) []byte {

	b := mua.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, mua.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'mua_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mua *mua_CM) FmtDateTimeMedium(t time.Time) []byte {

	b := mua.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, mua.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'mua_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mua *mua_CM) FmtDateTimeLong(t time.Time) []byte {

	b := mua.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, mua.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'mua_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mua *mua_CM) FmtDateTimeFull(t time.Time) []byte {

	b := mua.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, mua.FmtTimeFull(t)...)
