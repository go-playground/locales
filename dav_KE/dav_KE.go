...
// FmtDateTimeShort returns the short date & time representation of 't' for 'dav_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dav *dav_KE) FmtDateTimeShort(t time.Time) []byte {

	b := dav.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, dav.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'dav_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dav *dav_KE) FmtDateTimeMedium(t time.Time) []byte {

	b := dav.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, dav.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'dav_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dav *dav_KE) FmtDateTimeLong(t time.Time) []byte {

	b := dav.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, dav.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'dav_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dav *dav_KE) FmtDateTimeFull(t time.Time) []byte {

	b := dav.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, dav.FmtTimeFull(t)...)
