...
// FmtDateTimeShort returns the short date & time representation of 't' for 'fa'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fa *fa) FmtDateTimeShort(t time.Time) []byte {

	b := fa.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, fa.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'fa'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fa *fa) FmtDateTimeMedium(t time.Time) []byte {

	b := fa.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, fa.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'fa'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fa *fa) FmtDateTimeLong(t time.Time) []byte {

	b := fa.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, fa.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'fa'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fa *fa) FmtDateTimeFull(t time.Time) []byte {

	b := fa.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, fa.FmtTimeFull(t)...)
