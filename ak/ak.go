...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ak'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ak *ak) FmtDateTimeShort(t time.Time) []byte {

	b := ak.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ak.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ak'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ak *ak) FmtDateTimeMedium(t time.Time) []byte {

	b := ak.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ak.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ak'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ak *ak) FmtDateTimeLong(t time.Time) []byte {

	b := ak.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ak.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ak'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ak *ak) FmtDateTimeFull(t time.Time) []byte {

	b := ak.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ak.FmtTimeFull(t)...)
