...
// FmtDateTimeShort returns the short date & time representation of 't' for 'se'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (se *se) FmtDateTimeShort(t time.Time) []byte {

	b := se.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, se.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'se'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (se *se) FmtDateTimeMedium(t time.Time) []byte {

	b := se.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, se.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'se'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (se *se) FmtDateTimeLong(t time.Time) []byte {

	b := se.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, se.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'se'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (se *se) FmtDateTimeFull(t time.Time) []byte {

	b := se.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, se.FmtTimeFull(t)...)
