...
// FmtDateTimeShort returns the short date & time representation of 't' for 'nn_NO'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nn *nn_NO) FmtDateTimeShort(t time.Time) []byte {

	b := nn.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, nn.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'nn_NO'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nn *nn_NO) FmtDateTimeMedium(t time.Time) []byte {

	b := nn.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, nn.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'nn_NO'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nn *nn_NO) FmtDateTimeLong(t time.Time) []byte {

	b := nn.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, nn.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'nn_NO'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nn *nn_NO) FmtDateTimeFull(t time.Time) []byte {

	b := nn.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, nn.FmtTimeFull(t)...)
