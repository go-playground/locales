...
// FmtDateTimeShort returns the short date & time representation of 't' for 'nnh_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nnh *nnh_CM) FmtDateTimeShort(t time.Time) []byte {

	b := nnh.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, nnh.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'nnh_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nnh *nnh_CM) FmtDateTimeMedium(t time.Time) []byte {

	b := nnh.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, nnh.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'nnh_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nnh *nnh_CM) FmtDateTimeLong(t time.Time) []byte {

	b := nnh.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, nnh.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'nnh_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nnh *nnh_CM) FmtDateTimeFull(t time.Time) []byte {

	b := nnh.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, nnh.FmtTimeFull(t)...)
