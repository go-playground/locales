...
func (id *id) FmtNumber(num float64, v uint64) []byte {
	...
	for i := len(s) - 1; i >= 0; i-- {
		...
		if s[i] == '.' {
			...
			for j := len(id.decimal) - 1; j >= 0; j-- {
				b = append(b, id.decimal[j])
			}
		...
		if inWhole {
			...
			if count == 3 {
				...
				for j := len(id.group) - 1; j >= 0; j-- {
					b = append(b, id.group[j])
				}
