...
// FmtDateTimeShort returns the short date & time representation of 't' for 'mer'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mer *mer) FmtDateTimeShort(t time.Time) []byte {

	b := mer.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, mer.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'mer'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mer *mer) FmtDateTimeMedium(t time.Time) []byte {

	b := mer.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, mer.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'mer'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mer *mer) FmtDateTimeLong(t time.Time) []byte {

	b := mer.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, mer.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'mer'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mer *mer) FmtDateTimeFull(t time.Time) []byte {

	b := mer.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, mer.FmtTimeFull(t)...)
