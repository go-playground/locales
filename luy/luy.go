...
// FmtDateTimeShort returns the short date & time representation of 't' for 'luy'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (luy *luy) FmtDateTimeShort(t time.Time) []byte {

	b := luy.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, luy.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'luy'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (luy *luy) FmtDateTimeMedium(t time.Time) []byte {

	b := luy.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, luy.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'luy'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (luy *luy) FmtDateTimeLong(t time.Time) []byte {

	b := luy.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, luy.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'luy'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (luy *luy) FmtDateTimeFull(t time.Time) []byte {

	b := luy.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, luy.FmtTimeFull(t)...)
