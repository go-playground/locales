...
// FmtDateTimeShort returns the short date & time representation of 't' for 'pt_ST'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (pt *pt_ST) FmtDateTimeShort(t time.Time) []byte {

	b := pt.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, pt.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'pt_ST'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (pt *pt_ST) FmtDateTimeMedium(t time.Time) []byte {

	b := pt.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, pt.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'pt_ST'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (pt *pt_ST) FmtDateTimeLong(t time.Time) []byte {

	b := pt.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, pt.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'pt_ST'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (pt *pt_ST) FmtDateTimeFull(t time.Time) []byte {

	b := pt.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, pt.FmtTimeFull(t)...)
