...
// FmtDateTimeShort returns the short date & time representation of 't' for 'yi_001'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (yi *yi_001) FmtDateTimeShort(t time.Time) []byte {

	b := yi.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, yi.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'yi_001'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (yi *yi_001) FmtDateTimeMedium(t time.Time) []byte {

	b := yi.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, yi.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'yi_001'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (yi *yi_001) FmtDateTimeLong(t time.Time) []byte {

	b := yi.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, yi.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'yi_001'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (yi *yi_001) FmtDateTimeFull(t time.Time) []byte {

	b := yi.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, yi.FmtTimeFull(t)...)
