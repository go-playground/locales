...
// FmtDateTimeShort returns the short date & time representation of 't' for 'dz'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dz *dz) FmtDateTimeShort(t time.Time) []byte {

	b := dz.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, dz.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'dz'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dz *dz) FmtDateTimeMedium(t time.Time) []byte {

	b := dz.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, dz.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'dz'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dz *dz) FmtDateTimeLong(t time.Time) []byte {

	b := dz.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, dz.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'dz'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dz *dz) FmtDateTimeFull(t time.Time) []byte {

	b := dz.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, dz.FmtTimeFull(t)...)
