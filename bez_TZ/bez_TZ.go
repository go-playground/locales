...
// FmtDateTimeShort returns the short date & time representation of 't' for 'bez_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bez *bez_TZ) FmtDateTimeShort(t time.Time) []byte {

	b := bez.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, bez.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'bez_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bez *bez_TZ) FmtDateTimeMedium(t time.Time) []byte {

	b := bez.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, bez.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'bez_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bez *bez_TZ) FmtDateTimeLong(t time.Time) []byte {

	b := bez.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, bez.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'bez_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bez *bez_TZ) FmtDateTimeFull(t time.Time) []byte {

	b := bez.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, bez.FmtTimeFull(t)...)
