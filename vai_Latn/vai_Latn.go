...
func (vai *vai_Latn) FmtNumber(num float64, v uint64) []byte {
	...
	for i := len(s) - 1; i >= 0; i-- {
		...
		if s[i] == '.' {
			...
			for j := len(vai.decimal) - 1; j >= 0; j-- {
				b = append(b, vai.decimal[j])
			}
		...
		if inWhole {
			...
			if count == 3 {
				...
				for j := len(vai.group) - 1; j >= 0; j-- {
					b = append(b, vai.group[j])
				}
