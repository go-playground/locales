...
// FmtDateTimeShort returns the short date & time representation of 't' for 'yue_HK'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (yue *yue_HK) FmtDateTimeShort(t time.Time) []byte {

	b := yue.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, yue.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'yue_HK'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (yue *yue_HK) FmtDateTimeMedium(t time.Time) []byte {

	b := yue.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, yue.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'yue_HK'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (yue *yue_HK) FmtDateTimeLong(t time.Time) []byte {

	b := yue.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, yue.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'yue_HK'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (yue *yue_HK) FmtDateTimeFull(t time.Time) []byte {

	b := yue.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, yue.FmtTimeFull(t)...)
