...
// FmtDateTimeShort returns the short date & time representation of 't' for 'rn'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rn *rn) FmtDateTimeShort(t time.Time) []byte {

	b := rn.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, rn.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'rn'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rn *rn) FmtDateTimeMedium(t time.Time) []byte {

	b := rn.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, rn.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'rn'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rn *rn) FmtDateTimeLong(t time.Time) []byte {

	b := rn.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, rn.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'rn'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rn *rn) FmtDateTimeFull(t time.Time) []byte {

	b := rn.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, rn.FmtTimeFull(t)...)
