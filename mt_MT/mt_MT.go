...
// FmtDateTimeShort returns the short date & time representation of 't' for 'mt_MT'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mt *mt_MT) FmtDateTimeShort(t time.Time) []byte {

	b := mt.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, mt.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'mt_MT'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mt *mt_MT) FmtDateTimeMedium(t time.Time) []byte {

	b := mt.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, mt.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'mt_MT'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mt *mt_MT) FmtDateTimeLong(t time.Time) []byte {

	b := mt.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, mt.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'mt_MT'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mt *mt_MT) FmtDateTimeFull(t time.Time) []byte {

	b := mt.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, mt.FmtTimeFull(t)...)
