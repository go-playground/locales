...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ses_ML'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ses *ses_ML) FmtDateTimeShort(t time.Time) []byte {

	b := ses.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ses.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ses_ML'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ses *ses_ML) FmtDateTimeMedium(t time.Time) []byte {

	b := ses.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ses.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ses_ML'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ses *ses_ML) FmtDateTimeLong(t time.Time) []byte {

	b := ses.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ses.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ses_ML'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ses *ses_ML) FmtDateTimeFull(t time.Time) []byte {

	b := ses.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ses.FmtTimeFull(t)...)
