...
// FmtDateTimeShort returns the short date & time representation of 't' for 'os_RU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (os *os_RU) FmtDateTimeShort(t time.Time) []byte {

	b := os.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, os.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'os_RU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (os *os_RU) FmtDateTimeMedium(t time.Time) []byte {

	b := os.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, os.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'os_RU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (os *os_RU) FmtDateTimeLong(t time.Time) []byte {

	b := os.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, os.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'os_RU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (os *os_RU) FmtDateTimeFull(t time.Time) []byte {

	b := os.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, os.FmtTimeFull(t)...)
