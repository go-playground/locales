...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ta_SG'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ta *ta_SG) FmtDateTimeShort(t time.Time) []byte {

	b := ta.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ta.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ta_SG'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ta *ta_SG) FmtDateTimeMedium(t time.Time) []byte {

	b := ta.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ta.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ta_SG'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ta *ta_SG) FmtDateTimeLong(t time.Time) []byte {

	b := ta.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ta.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ta_SG'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ta *ta_SG) FmtDateTimeFull(t time.Time) []byte {

	b := ta.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ta.FmtTimeFull(t)...)
