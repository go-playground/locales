...
// FmtDateTimeShort returns the short date & time representation of 't' for 'uk'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (uk *uk) FmtDateTimeShort(t time.Time) []byte {

	b := uk.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, uk.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'uk'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (uk *uk) FmtDateTimeMedium(t time.Time) []byte {

	b := uk.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, uk.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'uk'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (uk *uk) FmtDateTimeLong(t time.Time) []byte {

	b := uk.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, uk.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'uk'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (uk *uk) FmtDateTimeFull(t time.Time) []byte {

	b := uk.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, uk.FmtTimeFull(t)...)
