...
// FmtDateTimeShort returns the short date & time representation of 't' for 'lag_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lag *lag_TZ) FmtDateTimeShort(t time.Time) []byte {

	b := lag.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, lag.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'lag_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lag *lag_TZ) FmtDateTimeMedium(t time.Time) []byte {

	b := lag.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, lag.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'lag_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lag *lag_TZ) FmtDateTimeLong(t time.Time) []byte {

	b := lag.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, lag.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'lag_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lag *lag_TZ) FmtDateTimeFull(t time.Time) []byte {

	b := lag.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, lag.FmtTimeFull(t)...)
