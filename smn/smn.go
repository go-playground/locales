...
// FmtDateTimeShort returns the short date & time representation of 't' for 'smn'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (smn *smn) FmtDateTimeShort(t time.Time) []byte {

	b := smn.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, smn.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'smn'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (smn *smn) FmtDateTimeMedium(t time.Time) []byte {

	b := smn.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, smn.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'smn'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (smn *smn) FmtDateTimeLong(t time.Time) []byte {

	b := smn.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, smn.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'smn'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (smn *smn) FmtDateTimeFull(t time.Time) []byte {

	b := smn.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, smn.FmtTimeFull(t)...)
