...
// FmtDateTimeShort returns the short date & time representation of 't' for 'asa'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (asa *asa) FmtDateTimeShort(t time.Time) []byte {

	b := asa.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, asa.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'asa'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (asa *asa) FmtDateTimeMedium(t time.Time) []byte {

	b := asa.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, asa.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'asa'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (asa *asa) FmtDateTimeLong(t time.Time) []byte {

	b := asa.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, asa.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'asa'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (asa *asa) FmtDateTimeFull(t time.Time) []byte {

	b := asa.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, asa.FmtTimeFull(t)...)
