...
// FmtDateTimeShort returns the short date & time representation of 't' for 'mgh_MZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mgh *mgh_MZ) FmtDateTimeShort(t time.Time) []byte {

	b := mgh.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, mgh.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'mgh_MZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mgh *mgh_MZ) FmtDateTimeMedium(t time.Time) []byte {

	b := mgh.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, mgh.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'mgh_MZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mgh *mgh_MZ) FmtDateTimeLong(t time.Time) []byte {

	b := mgh.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, mgh.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'mgh_MZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mgh *mgh_MZ) FmtDateTimeFull(t time.Time) []byte {

	b := mgh.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, mgh.FmtTimeFull(t)...)
