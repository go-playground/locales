...
// FmtDateTimeShort returns the short date & time representation of 't' for 'tr_TR'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (tr *tr_TR) FmtDateTimeShort(t time.Time) []byte {

	b := tr.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, tr.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'tr_TR'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (tr *tr_TR) FmtDateTimeMedium(t time.Time) []byte {

	b := tr.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, tr.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'tr_TR'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (tr *tr_TR) FmtDateTimeLong(t time.Time) []byte {

	b := tr.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, tr.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'tr_TR'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (tr *tr_TR) FmtDateTimeFull(t time.Time) []byte {

	b := tr.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, tr.FmtTimeFull(t)...)
