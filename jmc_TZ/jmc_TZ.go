...
// FmtDateTimeShort returns the short date & time representation of 't' for 'jmc_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (jmc *jmc_TZ) FmtDateTimeShort(t time.Time) []byte {

	b := jmc.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, jmc.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'jmc_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (jmc *jmc_TZ) FmtDateTimeMedium(t time.Time) []byte {

	b := jmc.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, jmc.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'jmc_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (jmc *jmc_TZ) FmtDateTimeLong(t time.Time) []byte {

	b := jmc.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, jmc.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'jmc_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (jmc *jmc_TZ) FmtDateTimeFull(t time.Time) []byte {

	b := jmc.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, jmc.FmtTimeFull(t)...)
