...
// FmtDateTimeShort returns the short date & time representation of 't' for 'twq_NE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (twq *twq_NE) FmtDateTimeShort(t time.Time) []byte {

	b := twq.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, twq.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'twq_NE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (twq *twq_NE) FmtDateTimeMedium(t time.Time) []byte {

	b := twq.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, twq.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'twq_NE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (twq *twq_NE) FmtDateTimeLong(t time.Time) []byte {

	b := twq.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, twq.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'twq_NE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (twq *twq_NE) FmtDateTimeFull(t time.Time) []byte {

	b := twq.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, twq.FmtTimeFull(t)...)
