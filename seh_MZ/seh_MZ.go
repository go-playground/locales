...
// FmtDateTimeShort returns the short date & time representation of 't' for 'seh_MZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (seh *seh_MZ) FmtDateTimeShort(t time.Time) []byte {

	b := seh.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, seh.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'seh_MZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (seh *seh_MZ) FmtDateTimeMedium(t time.Time) []byte {

	b := seh.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, seh.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'seh_MZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (seh *seh_MZ) FmtDateTimeLong(t time.Time) []byte {

	b := seh.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, seh.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'seh_MZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (seh *seh_MZ) FmtDateTimeFull(t time.Time) []byte {

	b := seh.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, seh.FmtTimeFull(t)...)
