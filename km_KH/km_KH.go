...
// FmtDateTimeShort returns the short date & time representation of 't' for 'km_KH'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (km *km_KH) FmtDateTimeShort(t time.Time) []byte {

	b := km.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, km.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'km_KH'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (km *km_KH) FmtDateTimeMedium(t time.Time) []byte {

	b := km.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, km.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'km_KH'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (km *km_KH) FmtDateTimeLong(t time.Time) []byte {

	b := km.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, km.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'km_KH'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (km *km_KH) FmtDateTimeFull(t time.Time) []byte {

	b := km.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, km.FmtTimeFull(t)...)
