...
func (ro *ro_MD) FmtNumber(num float64, v uint64) []byte {
	...
	for i := len(s) - 1; i >= 0; i-- {
		...
		if s[i] == '.' {
			...
			for j := len(ro.decimal) - 1; j >= 0; j-- {
				b = append(b, ro.decimal[j])
			}
		...
		if inWhole {
			...
			if count == 3 {
				...
				for j := len(ro.group) - 1; j >= 0; j-- {
					b = append(b, ro.group[j])
				}
