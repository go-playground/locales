...
// FmtDateTimeShort returns the short date & time representation of 't' for 'qu_BO'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (qu *qu_BO) FmtDateTimeShort(t time.Time) []byte {

	b := qu.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, qu.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'qu_BO'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (qu *qu_BO) FmtDateTimeMedium(t time.Time) []byte {

	b := qu.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, qu.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'qu_BO'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (qu *qu_BO) FmtDateTimeLong(t time.Time) []byte {

	b := qu.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, qu.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'qu_BO'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (qu *qu_BO) FmtDateTimeFull(t time.Time) []byte {

	b := qu.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, qu.FmtTimeFull(t)...)
