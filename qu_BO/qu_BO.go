...
func (qu *qu_BO) FmtNumber(num float64, v uint64) []byte {
	...
	for i := len(s) - 1; i >= 0; i-- {
		...
		if s[i] == '.' {
			...
			for j := len(qu.decimal) - 1; j >= 0; j-- {
				b = append(b, qu.decimal[j])
			}
		...
		if inWhole {
			...
			if count == 3 {
				...
				for j := len(qu.group) - 1; j >= 0; j-- {
					b = append(b, qu.group[j])
				}
