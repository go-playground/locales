...
// FmtDateTimeShort returns the short date & time representation of 't' for 'kab'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kab *kab) FmtDateTimeShort(t time.Time) []byte {

	b := kab.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, kab.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'kab'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kab *kab) FmtDateTimeMedium(t time.Time) []byte {

	b := kab.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, kab.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'kab'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kab *kab) FmtDateTimeLong(t time.Time) []byte {

	b := kab.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, kab.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'kab'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kab *kab) FmtDateTimeFull(t time.Time) []byte {

	b := kab.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, kab.FmtTimeFull(t)...)
