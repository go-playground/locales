...
// FmtDateTimeShort returns the short date & time representation of 't' for 'af_NA'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (af *af_NA) FmtDateTimeShort(t time.Time) []byte {

	b := af.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, af.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'af_NA'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (af *af_NA) FmtDateTimeMedium(t time.Time) []byte {

	b := af.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, af.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'af_NA'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (af *af_NA) FmtDateTimeLong(t time.Time) []byte {

	b := af.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, af.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'af_NA'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (af *af_NA) FmtDateTimeFull(t time.Time) []byte {

	b := af.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, af.FmtTimeFull(t)...)
