...
// FmtDateTimeShort returns the short date & time representation of 't' for 'sq'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sq *sq) FmtDateTimeShort(t time.Time) []byte {

	b := sq.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, sq.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'sq'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sq *sq) FmtDateTimeMedium(t time.Time) []byte {

	b := sq.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, sq.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'sq'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sq *sq) FmtDateTimeLong(t time.Time) []byte {

	b := sq.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, sq.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'sq'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sq *sq) FmtDateTimeFull(t time.Time) []byte {

	b := sq.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, sq.FmtTimeFull(t)...)
