...
// FmtDateTimeShort returns the short date & time representation of 't' for 'xog'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (xog *xog) FmtDateTimeShort(t time.Time) []byte {

	b := xog.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, xog.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'xog'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (xog *xog) FmtDateTimeMedium(t time.Time) []byte {

	b := xog.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, xog.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'xog'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (xog *xog) FmtDateTimeLong(t time.Time) []byte {

	b := xog.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, xog.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'xog'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (xog *xog) FmtDateTimeFull(t time.Time) []byte {

	b := xog.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, xog.FmtTimeFull(t)...)
