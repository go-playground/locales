...
// FmtDateTimeShort returns the short date & time representation of 't' for 'lkt_US'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lkt *lkt_US) FmtDateTimeShort(t time.Time) []byte {

	b := lkt.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, lkt.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'lkt_US'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lkt *lkt_US) FmtDateTimeMedium(t time.Time) []byte {

	b := lkt.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, lkt.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'lkt_US'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lkt *lkt_US) FmtDateTimeLong(t time.Time) []byte {

	b := lkt.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, lkt.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'lkt_US'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lkt *lkt_US) FmtDateTimeFull(t time.Time) []byte {

	b := lkt.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, lkt.FmtTimeFull(t)...)
