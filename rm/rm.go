...
// FmtDateTimeShort returns the short date & time representation of 't' for 'rm'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rm *rm) FmtDateTimeShort(t time.Time) []byte {

	b := rm.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, rm.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'rm'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rm *rm) FmtDateTimeMedium(t time.Time) []byte {

	b := rm.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, rm.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'rm'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rm *rm) FmtDateTimeLong(t time.Time) []byte {

	b := rm.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, rm.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'rm'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (rm *rm) FmtDateTimeFull(t time.Time) []byte {

	b := rm.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, rm.FmtTimeFull(t)...)
