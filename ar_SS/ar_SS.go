...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ar_SS'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ar *ar_SS) FmtDateTimeShort(t time.Time) []byte {

	b := ar.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ar.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ar_SS'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ar *ar_SS) FmtDateTimeMedium(t time.Time) []byte {

	b := ar.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ar.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ar_SS'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ar *ar_SS) FmtDateTimeLong(t time.Time) []byte {

	b := ar.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ar.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ar_SS'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ar *ar_SS) FmtDateTimeFull(t time.Time) []byte {

	b := ar.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ar.FmtTimeFull(t)...)
