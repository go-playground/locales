...
// FmtDateTimeShort returns the short date & time representation of 't' for 'bo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bo *bo) FmtDateTimeShort(t time.Time) []byte {

	b := bo.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, bo.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'bo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bo *bo) FmtDateTimeMedium(t time.Time) []byte {

	b := bo.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, bo.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'bo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bo *bo) FmtDateTimeLong(t time.Time) []byte {

	b := bo.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, bo.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'bo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bo *bo) FmtDateTimeFull(t time.Time) []byte {

	b := bo.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, bo.FmtTimeFull(t)...)
