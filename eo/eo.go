...
// FmtDateTimeShort returns the short date & time representation of 't' for 'eo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (eo *eo) FmtDateTimeShort(t time.Time) []byte {

	b := eo.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, eo.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'eo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (eo *eo) FmtDateTimeMedium(t time.Time) []byte {

	b := eo.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, eo.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'eo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (eo *eo) FmtDateTimeLong(t time.Time) []byte {

	b := eo.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, eo.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'eo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (eo *eo) FmtDateTimeFull(t time.Time) []byte {

	b := eo.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, eo.FmtTimeFull(t)...)
