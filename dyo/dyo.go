...
// FmtDateTimeShort returns the short date & time representation of 't' for 'dyo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dyo *dyo) FmtDateTimeShort(t time.Time) []byte {

	b := dyo.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, dyo.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'dyo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dyo *dyo) FmtDateTimeMedium(t time.Time) []byte {

	b := dyo.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, dyo.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'dyo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dyo *dyo) FmtDateTimeLong(t time.Time) []byte {

	b := dyo.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, dyo.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'dyo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dyo *dyo) FmtDateTimeFull(t time.Time) []byte {

	b := dyo.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, dyo.FmtTimeFull(t)...)
