...
// FmtDateTimeShort returns the short date & time representation of 't' for 'kkj_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kkj *kkj_CM) FmtDateTimeShort(t time.Time) []byte {

	b := kkj.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, kkj.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'kkj_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kkj *kkj_CM) FmtDateTimeMedium(t time.Time) []byte {

	b := kkj.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, kkj.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'kkj_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kkj *kkj_CM) FmtDateTimeLong(t time.Time) []byte {

	b := kkj.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, kkj.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'kkj_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kkj *kkj_CM) FmtDateTimeFull(t time.Time) []byte {

	b := kkj.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, kkj.FmtTimeFull(t)...)
