...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ln'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ln *ln) FmtDateTimeShort(t time.Time) []byte {

	b := ln.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ln.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ln'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ln *ln) FmtDateTimeMedium(t time.Time) []byte {

	b := ln.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ln.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ln'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ln *ln) FmtDateTimeLong(t time.Time) []byte {

	b := ln.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ln.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ln'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ln *ln) FmtDateTimeFull(t time.Time) []byte {

	b := ln.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ln.FmtTimeFull(t)...)
