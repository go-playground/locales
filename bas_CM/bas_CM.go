...
// FmtDateTimeShort returns the short date & time representation of 't' for 'bas_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bas *bas_CM) FmtDateTimeShort(t time.Time) []byte {

	b := bas.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, bas.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'bas_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bas *bas_CM) FmtDateTimeMedium(t time.Time) []byte {

	b := bas.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, bas.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'bas_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bas *bas_CM) FmtDateTimeLong(t time.Time) []byte {

	b := bas.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, bas.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'bas_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bas *bas_CM) FmtDateTimeFull(t time.Time) []byte {

	b := bas.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, bas.FmtTimeFull(t)...)
