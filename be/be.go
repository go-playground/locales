...
// FmtDateTimeShort returns the short date & time representation of 't' for 'be'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (be *be) FmtDateTimeShort(t time.Time) []byte {

	b := be.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, be.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'be'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (be *be) FmtDateTimeMedium(t time.Time) []byte {

	b := be.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, be.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'be'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (be *be) FmtDateTimeLong(t time.Time) []byte {

	b := be.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, be.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'be'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (be *be) FmtDateTimeFull(t time.Time) []byte {

	b := be.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, be.FmtTimeFull(t)...)
