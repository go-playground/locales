...
// FmtDateTimeShort returns the short date & time representation of 't' for 'fo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fo *fo) FmtDateTimeShort(t time.Time) []byte {

	b := fo.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, fo.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'fo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fo *fo) FmtDateTimeMedium(t time.Time) []byte {

	b := fo.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, fo.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'fo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fo *fo) FmtDateTimeLong(t time.Time) []byte {

	b := fo.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, fo.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'fo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fo *fo) FmtDateTimeFull(t time.Time) []byte {

	b := fo.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, fo.FmtTimeFull(t)...)
