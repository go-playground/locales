...
// FmtDateTimeShort returns the short date & time representation of 't' for 'mg'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mg *mg) FmtDateTimeShort(t time.Time) []byte {

	b := mg.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, mg.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'mg'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mg *mg) FmtDateTimeMedium(t time.Time) []byte {

	b := mg.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, mg.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'mg'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mg *mg) FmtDateTimeLong(t time.Time) []byte {

	b := mg.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, mg.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'mg'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mg *mg) FmtDateTimeFull(t time.Time) []byte {

	b := mg.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, mg.FmtTimeFull(t)...)
