...
// FmtDateTimeShort returns the short date & time representation of 't' for 'lv'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lv *lv) FmtDateTimeShort(t time.Time) []byte {

	b := lv.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, lv.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'lv'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lv *lv) FmtDateTimeMedium(t time.Time) []byte {

	b := lv.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, lv.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'lv'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lv *lv) FmtDateTimeLong(t time.Time) []byte {

	b := lv.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, lv.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'lv'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lv *lv) FmtDateTimeFull(t time.Time) []byte {

	b := lv.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, lv.FmtTimeFull(t)...)
