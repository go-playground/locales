...
func (lv *lv) FmtNumber(num float64, v uint64) []byte {
	...
	for i := len(s) - 1; i >= 0; i-- {
		...
		if s[i] == '.' {
			...
			for j := len(lv.decimal) - 1; j >= 0; j-- {
				b = append(b, lv.decimal[j])
			}
