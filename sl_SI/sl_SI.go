...
// FmtDateTimeShort returns the short date & time representation of 't' for 'sl_SI'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sl *sl_SI) FmtDateTimeShort(t time.Time) []byte {

	b := sl.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, sl.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'sl_SI'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sl *sl_SI) FmtDateTimeMedium(t time.Time) []byte {

	b := sl.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, sl.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'sl_SI'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sl *sl_SI) FmtDateTimeLong(t time.Time) []byte {

	b := sl.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, sl.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'sl_SI'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sl *sl_SI) FmtDateTimeFull(t time.Time) []byte {

	b := sl.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, sl.FmtTimeFull(t)...)
