...
// FmtDateTimeShort returns the short date & time representation of 't' for 'lrc'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lrc *lrc) FmtDateTimeShort(t time.Time) []byte {

	b := lrc.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, lrc.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'lrc'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lrc *lrc) FmtDateTimeMedium(t time.Time) []byte {

	b := lrc.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, lrc.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'lrc'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lrc *lrc) FmtDateTimeLong(t time.Time) []byte {

	b := lrc.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, lrc.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'lrc'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lrc *lrc) FmtDateTimeFull(t time.Time) []byte {

	b := lrc.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, lrc.FmtTimeFull(t)...)
