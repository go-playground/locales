...
// FmtDateTimeShort returns the short date & time representation of 't' for 'mgo_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mgo *mgo_CM) FmtDateTimeShort(t time.Time) []byte {

	b := mgo.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, mgo.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'mgo_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mgo *mgo_CM) FmtDateTimeMedium(t time.Time) []byte {

	b := mgo.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, mgo.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'mgo_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mgo *mgo_CM) FmtDateTimeLong(t time.Time) []byte {

	b := mgo.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, mgo.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'mgo_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mgo *mgo_CM) FmtDateTimeFull(t time.Time) []byte {

	b := mgo.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, mgo.FmtTimeFull(t)...)
