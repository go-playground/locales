...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ig_NG'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ig *ig_NG) FmtDateTimeShort(t time.Time) []byte {

	b := ig.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ig.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ig_NG'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ig *ig_NG) FmtDateTimeMedium(t time.Time) []byte {

	b := ig.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ig.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ig_NG'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ig *ig_NG) FmtDateTimeLong(t time.Time) []byte {

	b := ig.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ig.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ig_NG'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ig *ig_NG) FmtDateTimeFull(t time.Time) []byte {

	b := ig.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ig.FmtTimeFull(t)...)
