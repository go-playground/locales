...
// FmtDateTimeShort returns the short date & time representation of 't' for 'mn'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mn *mn) FmtDateTimeShort(t time.Time) []byte {

	b := mn.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, mn.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'mn'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mn *mn) FmtDateTimeMedium(t time.Time) []byte {

	b := mn.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, mn.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'mn'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mn *mn) FmtDateTimeLong(t time.Time) []byte {

	b := mn.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, mn.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'mn'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mn *mn) FmtDateTimeFull(t time.Time) []byte {

	b := mn.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, mn.FmtTimeFull(t)...)
