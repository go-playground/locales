...
// FmtDateTimeShort returns the short date & time representation of 't' for 'dsb'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dsb *dsb) FmtDateTimeShort(t time.Time) []byte {

	b := dsb.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, dsb.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'dsb'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dsb *dsb) FmtDateTimeMedium(t time.Time) []byte {

	b := dsb.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, dsb.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'dsb'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dsb *dsb) FmtDateTimeLong(t time.Time) []byte {

	b := dsb.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, dsb.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'dsb'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dsb *dsb) FmtDateTimeFull(t time.Time) []byte {

	b := dsb.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, dsb.FmtTimeFull(t)...)
