...
// FmtDateTimeShort returns the short date & time representation of 't' for 'pa_Arab'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (pa *pa_Arab) FmtDateTimeShort(t time.Time) []byte {

	b := pa.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, pa.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'pa_Arab'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (pa *pa_Arab) FmtDateTimeMedium(t time.Time) []byte {

	b := pa.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, pa.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'pa_Arab'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (pa *pa_Arab) FmtDateTimeLong(t time.Time) []byte {

	b := pa.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, pa.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'pa_Arab'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (pa *pa_Arab) FmtDateTimeFull(t time.Time) []byte {

	b := pa.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, pa.FmtTimeFull(t)...)
