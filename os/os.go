...
func (os *os) FmtNumber(num float64, v uint64) []byte {
	...
	for i := len(s) - 1; i >= 0; i-- {
		...
		if s[i] == '.' {
			...
			for j := len(os.decimal) - 1; j >= 0; j-- {
				b = append(b, os.decimal[j])
			}
