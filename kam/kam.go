...
// FmtDateTimeShort returns the short date & time representation of 't' for 'kam'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kam *kam) FmtDateTimeShort(t time.Time) []byte {

	b := kam.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, kam.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'kam'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kam *kam) FmtDateTimeMedium(t time.Time) []byte {

	b := kam.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, kam.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'kam'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kam *kam) FmtDateTimeLong(t time.Time) []byte {

	b := kam.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, kam.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'kam'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kam *kam) FmtDateTimeFull(t time.Time) []byte {

	b := kam.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, kam.FmtTimeFull(t)...)
