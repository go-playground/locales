...
// FmtDateTimeShort returns the short date & time representation of 't' for 'gv_IM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (gv *gv_IM) FmtDateTimeShort(t time.Time) []byte {

	b := gv.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, gv.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'gv_IM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (gv *gv_IM) FmtDateTimeMedium(t time.Time) []byte {

	b := gv.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, gv.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'gv_IM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (gv *gv_IM) FmtDateTimeLong(t time.Time) []byte {

	b := gv.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, gv.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'gv_IM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (gv *gv_IM) FmtDateTimeFull(t time.Time) []byte {

	b := gv.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, gv.FmtTimeFull(t)...)
