...
// FmtDateTimeShort returns the short date & time representation of 't' for 'cu_RU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (cu *cu_RU) FmtDateTimeShort(t time.Time) []byte {

	b := cu.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, cu.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'cu_RU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (cu *cu_RU) FmtDateTimeMedium(t time.Time) []byte {

	b := cu.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, cu.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'cu_RU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (cu *cu_RU) FmtDateTimeLong(t time.Time) []byte {

	b := cu.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, cu.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'cu_RU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (cu *cu_RU) FmtDateTimeFull(t time.Time) []byte {

	b := cu.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, cu.FmtTimeFull(t)...)
