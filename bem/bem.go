...
// FmtDateTimeShort returns the short date & time representation of 't' for 'bem'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bem *bem) FmtDateTimeShort(t time.Time) []byte {

	b := bem.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, bem.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'bem'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bem *bem) FmtDateTimeMedium(t time.Time) []byte {

	b := bem.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, bem.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'bem'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bem *bem) FmtDateTimeLong(t time.Time) []byte {

	b := bem.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, bem.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'bem'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bem *bem) FmtDateTimeFull(t time.Time) []byte {

	b := bem.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, bem.FmtTimeFull(t)...)
