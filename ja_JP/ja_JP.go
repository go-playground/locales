...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ja_JP'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ja *ja_JP) FmtDateTimeShort(t time.Time) []byte {

	b := ja.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ja.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ja_JP'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ja *ja_JP) FmtDateTimeMedium(t time.Time) []byte {

	b := ja.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ja.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ja_JP'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ja *ja_JP) FmtDateTimeLong(t time.Time) []byte {

	b := ja.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ja.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ja_JP'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ja *ja_JP) FmtDateTimeFull(t time.Time) []byte {

	b := ja.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ja.FmtTimeFull(t)...)
