...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ca'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ca *ca) FmtDateTimeShort(t time.Time) []byte {

	b := ca.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ca.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ca'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ca *ca) FmtDateTimeMedium(t time.Time) []byte {

	b := ca.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ca.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ca'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ca *ca) FmtDateTimeLong(t time.Time) []byte {

	b := ca.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ca.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ca'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ca *ca) FmtDateTimeFull(t time.Time) []byte {

	b := ca.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ca.FmtTimeFull(t)...)
