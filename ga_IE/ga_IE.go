...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ga_IE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ga *ga_IE) FmtDateTimeShort(t time.Time) []byte {

	b := ga.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ga.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ga_IE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ga *ga_IE) FmtDateTimeMedium(t time.Time) []byte {

	b := ga.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ga.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ga_IE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ga *ga_IE) FmtDateTimeLong(t time.Time) []byte {

	b := ga.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ga.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ga_IE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ga *ga_IE) FmtDateTimeFull(t time.Time) []byte {

	b := ga.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ga.FmtTimeFull(t)...)
