...
// FmtDateTimeShort returns the short date & time representation of 't' for 'sn_ZW'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sn *sn_ZW) FmtDateTimeShort(t time.Time) []byte {

	b := sn.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, sn.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'sn_ZW'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sn *sn_ZW) FmtDateTimeMedium(t time.Time) []byte {

	b := sn.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, sn.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'sn_ZW'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sn *sn_ZW) FmtDateTimeLong(t time.Time) []byte {

	b := sn.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, sn.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'sn_ZW'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sn *sn_ZW) FmtDateTimeFull(t time.Time) []byte {

	b := sn.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, sn.FmtTimeFull(t)...)
