...
// FmtDateTimeShort returns the short date & time representation of 't' for 'sbp_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sbp *sbp_TZ) FmtDateTimeShort(t time.Time) []byte {

	b := sbp.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, sbp.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'sbp_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sbp *sbp_TZ) FmtDateTimeMedium(t time.Time) []byte {

	b := sbp.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, sbp.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'sbp_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sbp *sbp_TZ) FmtDateTimeLong(t time.Time) []byte {

	b := sbp.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, sbp.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'sbp_TZ'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sbp *sbp_TZ) FmtDateTimeFull(t time.Time) []byte {

	b := sbp.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, sbp.FmtTimeFull(t)...)
