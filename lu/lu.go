...
// FmtDateTimeShort returns the short date & time representation of 't' for 'lu'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lu *lu) FmtDateTimeShort(t time.Time) []byte {

	b := lu.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, lu.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'lu'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lu *lu) FmtDateTimeMedium(t time.Time) []byte {

	b := lu.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, lu.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'lu'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lu *lu) FmtDateTimeLong(t time.Time) []byte {

	b := lu.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, lu.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'lu'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lu *lu) FmtDateTimeFull(t time.Time) []byte {

	b := lu.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, lu.FmtTimeFull(t)...)
