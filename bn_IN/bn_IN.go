...
// FmtDateTimeShort returns the short date & time representation of 't' for 'bn_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bn *bn_IN) FmtDateTimeShort(t time.Time) []byte {

	b := bn.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, bn.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'bn_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bn *bn_IN) FmtDateTimeMedium(t time.Time) []byte {

	b := bn.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, bn.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'bn_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bn *bn_IN) FmtDateTimeLong(t time.Time) []byte {

	b := bn.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, bn.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'bn_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bn *bn_IN) FmtDateTimeFull(t time.Time) []byte {

	b := bn.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, bn.FmtTimeFull(t)...)
