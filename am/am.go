...
// FmtDateTimeShort returns the short date & time representation of 't' for 'am'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (am *am) FmtDateTimeShort(t time.Time) []byte {

	b := am.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, am.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'am'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (am *am) FmtDateTimeMedium(t time.Time) []byte {

	b := am.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, am.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'am'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (am *am) FmtDateTimeLong(t time.Time) []byte {

	b := am.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, am.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'am'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (am *am) FmtDateTimeFull(t time.Time) []byte {

	b := am.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, am.FmtTimeFull(t)...)
