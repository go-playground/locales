...
func (am *am) FmtNumber(num float64, v uint64) []byte {
	...
	for i := len(s) - 1; i >= 0; i-- {
		...
		if s[i] == '.' {
			...
			for j := len(am.decimal) - 1; j >= 0; j-- {
				b = append(b, am.decimal[j])
			}
		...
		if inWhole {
			...
			if count == 3 {
				...
				for j := len(am.group) - 1; j >= 0; j-- {
					b = append(b, am.group[j])
				}
