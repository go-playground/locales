...
// FmtDateTimeShort returns the short date & time representation of 't' for 'fur_IT'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fur *fur_IT) FmtDateTimeShort(t time.Time) []byte {

	b := fur.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, fur.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'fur_IT'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fur *fur_IT) FmtDateTimeMedium(t time.Time) []byte {

	b := fur.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, fur.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'fur_IT'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fur *fur_IT) FmtDateTimeLong(t time.Time) []byte {

	b := fur.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, fur.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'fur_IT'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (fur *fur_IT) FmtDateTimeFull(t time.Time) []byte {

	b := fur.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, fur.FmtTimeFull(t)...)
