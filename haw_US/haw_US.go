...
// FmtDateTimeShort returns the short date & time representation of 't' for 'haw_US'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (haw *haw_US) FmtDateTimeShort(t time.Time) []byte {

	b := haw.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, haw.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'haw_US'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (haw *haw_US) FmtDateTimeMedium(t time.Time) []byte {

	b := haw.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, haw.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'haw_US'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (haw *haw_US) FmtDateTimeLong(t time.Time) []byte {

	b := haw.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, haw.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'haw_US'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (haw *haw_US) FmtDateTimeFull(t time.Time) []byte {

	b := haw.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, haw.FmtTimeFull(t)...)
