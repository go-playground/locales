...
// FmtDateTimeShort returns the short date & time representation of 't' for 'sah_RU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sah *sah_RU) FmtDateTimeShort(t time.Time) []byte {

	b := sah.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, sah.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'sah_RU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sah *sah_RU) FmtDateTimeMedium(t time.Time) []byte {

	b := sah.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, sah.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'sah_RU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sah *sah_RU) FmtDateTimeLong(t time.Time) []byte {

	b := sah.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, sah.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'sah_RU'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sah *sah_RU) FmtDateTimeFull(t time.Time) []byte {

	b := sah.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, sah.FmtTimeFull(t)...)
