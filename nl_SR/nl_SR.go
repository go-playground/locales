...
// FmtDateTimeShort returns the short date & time representation of 't' for 'nl_SR'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nl *nl_SR) FmtDateTimeShort(t time.Time) []byte {

	b := nl.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, nl.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'nl_SR'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nl *nl_SR) FmtDateTimeMedium(t time.Time) []byte {

	b := nl.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, nl.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'nl_SR'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nl *nl_SR) FmtDateTimeLong(t time.Time) []byte {

	b := nl.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, nl.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'nl_SR'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (nl *nl_SR) FmtDateTimeFull(t time.Time) []byte {

	b := nl.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, nl.FmtTimeFull(t)...)
