...
// FmtDateTimeShort returns the short date & time representation of 't' for 'mk'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mk *mk) FmtDateTimeShort(t time.Time) []byte {

	b := mk.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, mk.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'mk'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mk *mk) FmtDateTimeMedium(t time.Time) []byte {

	b := mk.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, mk.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'mk'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mk *mk) FmtDateTimeLong(t time.Time) []byte {

	b := mk.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, mk.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'mk'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mk *mk) FmtDateTimeFull(t time.Time) []byte {

	b := mk.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, mk.FmtTimeFull(t)...)
