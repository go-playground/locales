...
// FmtDateTimeShort returns the short date & time representation of 't' for 'kln_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kln *kln_KE) FmtDateTimeShort(t time.Time) []byte {

	b := kln.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, kln.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'kln_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kln *kln_KE) FmtDateTimeMedium(t time.Time) []byte {

	b := kln.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, kln.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'kln_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kln *kln_KE) FmtDateTimeLong(t time.Time) []byte {

	b := kln.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, kln.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'kln_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kln *kln_KE) FmtDateTimeFull(t time.Time) []byte {

	b := kln.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, kln.FmtTimeFull(t)...)
