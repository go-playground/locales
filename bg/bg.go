...
// FmtDateTimeShort returns the short date & time representation of 't' for 'bg'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bg *bg) FmtDateTimeShort(t time.Time) []byte {

	b := bg.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, bg.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'bg'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bg *bg) FmtDateTimeMedium(t time.Time) []byte {

	b := bg.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, bg.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'bg'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bg *bg) FmtDateTimeLong(t time.Time) []byte {

	b := bg.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, bg.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'bg'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (bg *bg) FmtDateTimeFull(t time.Time) []byte {

	b := bg.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, bg.FmtTimeFull(t)...)
