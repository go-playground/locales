...
// FmtDateTimeShort returns the short date & time representation of 't' for 'so'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (so *so) FmtDateTimeShort(t time.Time) []byte {

	b := so.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, so.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'so'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (so *so) FmtDateTimeMedium(t time.Time) []byte {

	b := so.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, so.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'so'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (so *so) FmtDateTimeLong(t time.Time) []byte {

	b := so.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, so.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'so'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (so *so) FmtDateTimeFull(t time.Time) []byte {

	b := so.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, so.FmtTimeFull(t)...)
