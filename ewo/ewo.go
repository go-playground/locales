...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ewo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ewo *ewo) FmtDateTimeShort(t time.Time) []byte {

	b := ewo.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ewo.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ewo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ewo *ewo) FmtDateTimeMedium(t time.Time) []byte {

	b := ewo.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ewo.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ewo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ewo *ewo) FmtDateTimeLong(t time.Time) []byte {

	b := ewo.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ewo.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ewo'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ewo *ewo) FmtDateTimeFull(t time.Time) []byte {

	b := ewo.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ewo.FmtTimeFull(t)...)
