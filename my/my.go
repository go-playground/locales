...
func (my *my) FmtNumber(num float64, v uint64) []byte {
	...
	for i := len(s) - 1; i >= 0; i-- {
		...
		if s[i] == '.' {
			...
			for j := len(my.decimal) - 1; j >= 0; j-- {
				b = append(b, my.decimal[j])
			}
		...
		if inWhole {
			...
			if count == 3 {
				...
				for j := len(my.group) - 1; j >= 0; j-- {
					b = append(b, my.group[j])
				}
