...
// FmtDateTimeShort returns the short date & time representation of 't' for 'saq'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (saq *saq) FmtDateTimeShort(t time.Time) []byte {

	b := saq.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, saq.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'saq'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (saq *saq) FmtDateTimeMedium(t time.Time) []byte {

	b := saq.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, saq.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'saq'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (saq *saq) FmtDateTimeLong(t time.Time) []byte {

	b := saq.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, saq.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'saq'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (saq *saq) FmtDateTimeFull(t time.Time) []byte {

	b := saq.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, saq.FmtTimeFull(t)...)
