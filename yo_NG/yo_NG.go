...
// FmtDateTimeShort returns the short date & time representation of 't' for 'yo_NG'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (yo *yo_NG) FmtDateTimeShort(t time.Time) []byte {

	b := yo.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, yo.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'yo_NG'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (yo *yo_NG) FmtDateTimeMedium(t time.Time) []byte {

	b := yo.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, yo.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'yo_NG'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (yo *yo_NG) FmtDateTimeLong(t time.Time) []byte {

	b := yo.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, yo.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'yo_NG'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (yo *yo_NG) FmtDateTimeFull(t time.Time) []byte {

	b := yo.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, yo.FmtTimeFull(t)...)
