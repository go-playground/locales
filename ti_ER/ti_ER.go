...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ti_ER'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ti *ti_ER) FmtDateTimeShort(t time.Time) []byte {

	b := ti.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ti.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ti_ER'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ti *ti_ER) FmtDateTimeMedium(t time.Time) []byte {

	b := ti.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ti.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ti_ER'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ti *ti_ER) FmtDateTimeLong(t time.Time) []byte {

	b := ti.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ti.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ti_ER'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ti *ti_ER) FmtDateTimeFull(t time.Time) []byte {

	b := ti.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ti.FmtTimeFull(t)...)
