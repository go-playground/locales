...
// FmtDateTimeShort returns the short date & time representation of 't' for 'or_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (or *or_IN) FmtDateTimeShort(t time.Time) []byte {

	b := or.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, or.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'or_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (or *or_IN) FmtDateTimeMedium(t time.Time) []byte {

	b := or.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, or.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'or_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (or *or_IN) FmtDateTimeLong(t time.Time) []byte {

	b := or.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, or.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'or_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (or *or_IN) FmtDateTimeFull(t time.Time) []byte {

	b := or.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, or.FmtTimeFull(t)...)
