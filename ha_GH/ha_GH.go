...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ha_GH'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ha *ha_GH) FmtDateTimeShort(t time.Time) []byte {

	b := ha.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ha.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ha_GH'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ha *ha_GH) FmtDateTimeMedium(t time.Time) []byte {

	b := ha.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ha.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ha_GH'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ha *ha_GH) FmtDateTimeLong(t time.Time) []byte {

	b := ha.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ha.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ha_GH'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ha *ha_GH) FmtDateTimeFull(t time.Time) []byte {

	b := ha.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ha.FmtTimeFull(t)...)
