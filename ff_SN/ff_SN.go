...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ff_SN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ff *ff_SN) FmtDateTimeShort(t time.Time) []byte {

	b := ff.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ff.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ff_SN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ff *ff_SN) FmtDateTimeMedium(t time.Time) []byte {

	b := ff.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ff.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ff_SN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ff *ff_SN) FmtDateTimeLong(t time.Time) []byte {

	b := ff.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ff.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ff_SN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ff *ff_SN) FmtDateTimeFull(t time.Time) []byte {

	b := ff.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ff.FmtTimeFull(t)...)
