...
// FmtDateTimeShort returns the short date & time representation of 't' for 'agq_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (agq *agq_CM) FmtDateTimeShort(t time.Time) []byte {

	b := agq.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, agq.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'agq_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (agq *agq_CM) FmtDateTimeMedium(t time.Time) []byte {

	b := agq.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, agq.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'agq_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (agq *agq_CM) FmtDateTimeLong(t time.Time) []byte {

	b := agq.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, agq.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'agq_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (agq *agq_CM) FmtDateTimeFull(t time.Time) []byte {

	b := agq.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, agq.FmtTimeFull(t)...)
