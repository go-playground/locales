...
// FmtDateTimeShort returns the short date & time representation of 't' for 'jgo_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (jgo *jgo_CM) FmtDateTimeShort(t time.Time) []byte {

	b := jgo.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, jgo.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'jgo_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (jgo *jgo_CM) FmtDateTimeMedium(t time.Time) []byte {

	b := jgo.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, jgo.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'jgo_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (jgo *jgo_CM) FmtDateTimeLong(t time.Time) []byte {

	b := jgo.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, jgo.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'jgo_CM'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (jgo *jgo_CM) FmtDateTimeFull(t time.Time) []byte {

	b := jgo.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, jgo.FmtTimeFull(t)...)
