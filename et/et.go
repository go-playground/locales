...
// FmtDateTimeShort returns the short date & time representation of 't' for 'et'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (et *et) FmtDateTimeShort(t time.Time) []byte {

	b := et.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, et.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'et'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (et *et) FmtDateTimeMedium(t time.Time) []byte {

	b := et.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, et.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'et'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (et *et) FmtDateTimeLong(t time.Time) []byte {

	b := et.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, et.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'et'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (et *et) FmtDateTimeFull(t time.Time) []byte {

	b := et.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, et.FmtTimeFull(t)...)
