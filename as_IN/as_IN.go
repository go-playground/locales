...
// FmtDateTimeShort returns the short date & time representation of 't' for 'as_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (as *as_IN) FmtDateTimeShort(t time.Time) []byte {

	b := as.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, as.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'as_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (as *as_IN) FmtDateTimeMedium(t time.Time) []byte {

	b := as.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, as.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'as_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (as *as_IN) FmtDateTimeLong(t time.Time) []byte {

	b := as.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, as.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'as_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (as *as_IN) FmtDateTimeFull(t time.Time) []byte {

	b := as.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, as.FmtTimeFull(t)...)
