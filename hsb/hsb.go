...
// FmtDateTimeShort returns the short date & time representation of 't' for 'hsb'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (hsb *hsb) FmtDateTimeShort(t time.Time) []byte {

	b := hsb.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, hsb.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'hsb'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (hsb *hsb) FmtDateTimeMedium(t time.Time) []byte {

	b := hsb.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, hsb.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'hsb'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (hsb *hsb) FmtDateTimeLong(t time.Time) []byte {

	b := hsb.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, hsb.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'hsb'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (hsb *hsb) FmtDateTimeFull(t time.Time) []byte {

	b := hsb.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, hsb.FmtTimeFull(t)...)
