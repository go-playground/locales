...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ii'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ii *ii) FmtDateTimeShort(t time.Time) []byte {

	b := ii.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ii.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ii'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ii *ii) FmtDateTimeMedium(t time.Time) []byte {

	b := ii.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ii.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ii'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ii *ii) FmtDateTimeLong(t time.Time) []byte {

	b := ii.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ii.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ii'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ii *ii) FmtDateTimeFull(t time.Time) []byte {

	b := ii.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ii.FmtTimeFull(t)...)
