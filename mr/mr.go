...
// FmtDateTimeShort returns the short date & time representation of 't' for 'mr'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mr *mr) FmtDateTimeShort(t time.Time) []byte {

	b := mr.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, mr.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'mr'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mr *mr) FmtDateTimeMedium(t time.Time) []byte {

	b := mr.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, mr.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'mr'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mr *mr) FmtDateTimeLong(t time.Time) []byte {

	b := mr.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, mr.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'mr'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (mr *mr) FmtDateTimeFull(t time.Time) []byte {

	b := mr.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, mr.FmtTimeFull(t)...)
