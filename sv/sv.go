...
// FmtDateTimeShort returns the short date & time representation of 't' for 'sv'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sv *sv) FmtDateTimeShort(t time.Time) []byte {

	b := sv.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, sv.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'sv'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sv *sv) FmtDateTimeMedium(t time.Time) []byte {

	b := sv.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, sv.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'sv'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sv *sv) FmtDateTimeLong(t time.Time) []byte {

	b := sv.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, sv.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'sv'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (sv *sv) FmtDateTimeFull(t time.Time) []byte {

	b := sv.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, sv.FmtTimeFull(t)...)
