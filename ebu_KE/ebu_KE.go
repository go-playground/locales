...
// FmtDateTimeShort returns the short date & time representation of 't' for 'ebu_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ebu *ebu_KE) FmtDateTimeShort(t time.Time) []byte {

	b := ebu.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, ebu.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'ebu_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ebu *ebu_KE) FmtDateTimeMedium(t time.Time) []byte {

	b := ebu.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, ebu.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'ebu_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ebu *ebu_KE) FmtDateTimeLong(t time.Time) []byte {

	b := ebu.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, ebu.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'ebu_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (ebu *ebu_KE) FmtDateTimeFull(t time.Time) []byte {

	b := ebu.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, ebu.FmtTimeFull(t)...)
