...
// FmtDateTimeShort returns the short date & time representation of 't' for 'luo_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (luo *luo_KE) FmtDateTimeShort(t time.Time) []byte {

	b := luo.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, luo.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'luo_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (luo *luo_KE) FmtDateTimeMedium(t time.Time) []byte {

	b := luo.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, luo.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'luo_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (luo *luo_KE) FmtDateTimeLong(t time.Time) []byte {

	b := luo.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, luo.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'luo_KE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (luo *luo_KE) FmtDateTimeFull(t time.Time) []byte {

	b := luo.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, luo.FmtTimeFull(t)...)
