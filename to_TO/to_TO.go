...
// FmtDateTimeShort returns the short date & time representation of 't' for 'to_TO'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (to *to_TO) FmtDateTimeShort(t time.Time) []byte {

	b := to.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, to.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'to_TO'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (to *to_TO) FmtDateTimeMedium(t time.Time) []byte {

	b := to.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, to.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'to_TO'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (to *to_TO) FmtDateTimeLong(t time.Time) []byte {

	b := to.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, to.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'to_TO'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (to *to_TO) FmtDateTimeFull(t time.Time) []byte {

	b := to.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, to.FmtTimeFull(t)...)
