...
// FmtDateTimeShort returns the short date & time representation of 't' for 'lt_LT'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lt *lt_LT) FmtDateTimeShort(t time.Time) []byte {

	b := lt.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, lt.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'lt_LT'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lt *lt_LT) FmtDateTimeMedium(t time.Time) []byte {

	b := lt.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, lt.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'lt_LT'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lt *lt_LT) FmtDateTimeLong(t time.Time) []byte {

	b := lt.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, lt.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'lt_LT'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (lt *lt_LT) FmtDateTimeFull(t time.Time) []byte {

	b := lt.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, lt.FmtTimeFull(t)...)
