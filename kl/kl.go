...
func (kl *kl) FmtNumber(num float64, v uint64) []byte {
	...
	for i := len(s) - 1; i >= 0; i-- {
		...
		if s[i] == '.' {
			...
			for j := len(kl.decimal) - 1; j >= 0; j-- {
				b = append(b, kl.decimal[j])
			}
		...
		if inWhole {
			...
			if count == 3 {
				...
				for j := len(kl.group) - 1; j >= 0; j-- {
					b = append(b, kl.group[j])
				}
