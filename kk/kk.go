...
// FmtDateTimeShort returns the short date & time representation of 't' for 'kk'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kk *kk) FmtDateTimeShort(t time.Time) []byte {

	b := kk.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, kk.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'kk'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kk *kk) FmtDateTimeMedium(t time.Time) []byte {

	b := kk.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, kk.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'kk'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kk *kk) FmtDateTimeLong(t time.Time) []byte {

	b := kk.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, kk.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'kk'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kk *kk) FmtDateTimeFull(t time.Time) []byte {

	b := kk.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, kk.FmtTimeFull(t)...)
