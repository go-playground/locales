...
// FmtDateTimeShort returns the short date & time representation of 't' for 'dje_NE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dje *dje_NE) FmtDateTimeShort(t time.Time) []byte {

	b := dje.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, dje.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'dje_NE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dje *dje_NE) FmtDateTimeMedium(t time.Time) []byte {

	b := dje.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, dje.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'dje_NE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dje *dje_NE) FmtDateTimeLong(t time.Time) []byte {

	b := dje.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, dje.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'dje_NE'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (dje *dje_NE) FmtDateTimeFull(t time.Time) []byte {

	b := dje.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, dje.FmtTimeFull(t)...)
