...
// FmtDateTimeShort returns the short date & time representation of 't' for 'zgh_MA'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (zgh *zgh_MA) FmtDateTimeShort(t time.Time) []byte {

	b := zgh.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, zgh.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'zgh_MA'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (zgh *zgh_MA) FmtDateTimeMedium(t time.Time) []byte {

	b := zgh.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, zgh.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'zgh_MA'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (zgh *zgh_MA) FmtDateTimeLong(t time.Time) []byte {

	b := zgh.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, zgh.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'zgh_MA'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (zgh *zgh_MA) FmtDateTimeFull(t time.Time) []byte {

	b := zgh.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, zgh.FmtTimeFull(t)...)
