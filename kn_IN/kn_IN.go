...
// FmtDateTimeShort returns the short date & time representation of 't' for 'kn_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kn *kn_IN) FmtDateTimeShort(t time.Time) []byte {

	b := kn.FmtDateShort(t)
	b = append(b, ' ')
	b = append(b, kn.FmtTimeShort(t)...)

	return b
}

// FmtDateTimeMedium returns the medium date & time representation of 't' for 'kn_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kn *kn_IN) FmtDateTimeMedium(t time.Time) []byte {

	b := kn.FmtDateMedium(t)
	b = append(b, ' ')
	b = append(b, kn.FmtTimeMedium(t)...)

	return b
}

// FmtDateTimeLong returns the long date & time representation of 't' for 'kn_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kn *kn_IN) FmtDateTimeLong(t time.Time) []byte {

	b := kn.FmtDateLong(t)
	b = append(b, ' ')
	b = append(b, kn.FmtTimeLong(t)...)

	return b
}

// FmtDateTimeFull returns the full date & time representation of 't' for 'kn_IN'
// returned as a []byte just in case the caller wishes to add more and can help
// avoid allocations; otherwise just cast as string.
func (kn *kn_IN) FmtDateTimeFull(t time.Time) []byte {

	b := kn.FmtDateFull(t)
	b = append(b, ' ')
	b = append(b, kn.FmtTimeFull(t)...)
